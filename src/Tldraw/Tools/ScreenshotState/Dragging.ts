...
export class ScreenshotDragging extends StateNode {
    ...
    // where the user started pointing and the place where their pointer is now.
    // If the user is holding Shift, then we modify the dimensions of this box 
    // so that it is in a 16:9 aspect ratio.
    override onEnter = () => {
        this.update()
    }
    ...
    // When the user makes a pointer up and stops dragging, 
    ...
    // When the user cancels (esc key) or makes a pointer up event, 
