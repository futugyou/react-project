...
} from '@/CytoscapeGraph/Contexts/CytoscapeContext'
...
import {
  ...
  getGridGuide,
} from '@/CytoscapeGraph/Layouting/CytoscapeLayout'
...
const CytoscapePanel = () => {
  ...
  const updateCytoscapeCore = useCallback(
    (cy: cytoscape.Core) => {
      dispatch({
  ...
  const cyCallback = useCallback(
    (cy: cytoscape.Core) => {
      ...
      cy.ready(() => {
        const removeHighlight = setTimeout(() => cy.elements().removeClass('highlight'), 2000)
        cyRef.current = cy
  ...
  return (
    <div className="cytoscapePanel">
      ...
      <CytoscapeController
        setVisible={(state) => setControllerVisible(state)}
