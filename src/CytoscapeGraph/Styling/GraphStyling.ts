export const graphStyle: cytoscape.StylesheetCSS | cytoscape.StylesheetCSS[] = [
  {
    selector: '.image',
    css: {
      label: 'data(label)',
      'font-family': 'Amazon Ember, Helvetica, Arial, sans-serif',
      'text-valign': 'bottom',
      'text-halign': 'center',
      'font-size': '5rem',
      'text-margin-y': 3,
      'background-color': '#fff',
      'background-image': function (ele: any) {
        return ele.data('image') ?? 'none'
      },
      'background-fit': 'cover',
      'background-opacity': 1,
      shape: 'ellipse',
      color: '#000',
      'background-width-relative-to': 'inner',
      'background-height-relative-to': 'inner',
      'background-width': '50%',
      'background-height': '50%',
      padding: '5px',
      'border-style': function (ele: any) {
        return ele.data('borderStyle') ?? 'solid'
      },
      'border-width': function (ele: any) {
        return ele.data('borderSize') ?? '1px'
      },
      'border-color': function (ele: any) {
        return ele.data('borderColour') ?? '#000'
      },
    },
  },
  {
    selector: '.vpc',
    css: {
      label: 'data(label)',
      'font-size': '7rem',
      'font-family': 'Amazon Ember, Helvetica, Arial, sans-serif',
      'text-valign': 'top',
      'text-halign': 'center',
      'background-color': '#683dc2',
      'background-opacity': 0.05,
      'text-margin-y': '-4.5%' as any,
      'border-style': 'solid',
      'border-width': 1,
      'border-color': '#683dc2',
      'text-border-width': 2,
      'text-background-opacity': 1,
      'text-border-color': '#683dc2',
      'text-background-color': '#683dc2',
      'text-background-shape': 'rectangle',
      'text-background-padding': '5px',
      shape: 'roundrectangle',
      color: '#fafafa',
    },
  },
  {
    selector: '.vpc:active',
    css: {
      label: 'data(label)',
      'overlay-color': '#683dc2',
      'overlay-opacity': 0.45,
      shape: 'roundrectangle',
      color: '#fafafa',
    },
  },
  {
    selector: '.vpc:selected',
    css: {
      label: 'data(label)',
      'overlay-color': '#683dc2',
      'overlay-opacity': 0.25,
      shape: 'roundrectangle',
      color: '#fafafa',
    },
  },
  {
    selector: '.subnet:active',
    css: {
      label: 'data(label)',
      'overlay-color': function (ele: any) {
        return ele.data('subnetColour') ?? '#fafafa'
      },
      'overlay-opacity': 0.45,
      shape: 'roundrectangle',
      color: '#fafafa',
    },
  },
  {
    selector: '.subnet:selected',
    css: {
      label: 'data(label)',
      'overlay-color': function (ele: any) {
        return ele.data('subnetColour') ?? '#fafafa'
      },
      'overlay-opacity': 0.25,
      shape: 'roundrectangle',
      color: '#fafafa',
    },
  },
  {
    selector: '.subnet',
    css: {
      label: 'data(label)',
      'font-size': '7rem',
      'font-family': 'Amazon Ember, Helvetica, Arial, sans-serif',
      'text-valign': 'top',
      'text-halign': 'center',
      'background-color': function (ele: any) {
        return ele.data('subnetColour') ?? '#fafafa'
      },
      'background-opacity': 0.05,
      'border-style': 'dashed',
      'border-width': 1,
      'text-background-color': function (ele: any) {
        return ele.data('subnetColour') ?? '#fafafa'
      },
      'border-color': function (ele: any) {
        return ele.data('subnetColour') ?? '#fafafa'
      },
      'text-border-color': function (ele: any) {
        return ele.data('subnetColour') ?? '#fafafa'
      },
      'text-background-opacity': 1,
      'text-background-shape': 'rectangle',
      'text-background-padding': '5px',
      'text-margin-y': '-4.5%' as any,
      'text-border-width': '2px' as any,
      shape: 'roundrectangle',
      color: '#fafafa',
    },
  },
  {
    selector: '.type:selected',
    css: {
      label: 'data(label)',
      'overlay-color': '#545B64',
      'overlay-opacity': 0.25,
      shape: 'roundrectangle',
      color: '#fafafa',
    },
  },
  {
    selector: '.type',
    css: {
      label: 'data(label)',
      'font-size': '6rem',
      'font-family': 'Amazon Ember, Helvetica, Arial, sans-serif',
      'text-valign': 'top',
      'text-halign': 'center',
      'background-color': '#545B64',
      'background-opacity': 0.05,
      'border-style': 'solid',
      'border-width': 1,
      'border-color': '#545B64',
      'text-margin-y': '-3%' as any,
      'text-border-width': '1px' as any,
      'text-background-opacity': 1,
      'text-border-color': '#545B64',
      'text-background-color': '#545B64',
      'text-background-shape': 'rectangle',
      'text-background-padding': '3px',
      shape: 'roundrectangle',
      color: '#fafafa',
    },
  },
  {
    selector: '.region:active',
    css: {
      label: 'data(label)',
      'overlay-color': function (ele: any) {
        return ele.data('regionColour') ?? '#fafafa'
      },
      'overlay-opacity': 0.45,
      shape: 'roundrectangle',
      color: '#fafafa',
    },
  },
  {
    selector: '.region:selected',
    css: {
      label: 'data(label)',
      'overlay-color': function (ele: any) {
        return ele.data('regionColour') ?? '#fafafa'
      },
      'overlay-opacity': 0.25,
      shape: 'roundrectangle',
      color: '#fafafa',
    },
  },
  {
    selector: '.region',
    css: {
      label: 'data(label)',
      'font-family': 'Amazon Ember, Helvetica, Arial, sans-serif',
      'font-size': '7rem',
      'text-valign': 'top',
      'text-halign': 'center',
      'background-color': function (ele: any) {
        return ele.data('color') ?? '#fafafa'
      },
      'background-opacity': function (ele: any) {
        return ele.data('opacity') ?? 1
      },
      'border-style': 'dashed',
      'border-width': 1,
      'border-color': function (ele: any) {
        return ele.data('regionColour') ?? '#fafafa'
      },
      'text-margin-y': '-5%' as any,
      'text-border-width': '2px' as any,
      'text-background-opacity': 1,
      'text-border-color': function (ele: any) {
        return ele.data('regionColour') ?? '#fafafa'
      },
      'text-background-color': function (ele: any) {
        return ele.data('regionColour') ?? '#fafafa'
      },
      'text-background-shape': 'rectangle',
      'text-background-padding': '5px',
      shape: 'rectangle',
      color: '#fafafa',
    },
  },

  {
    selector: '.availabilityZone:selected',
    css: {
      label: 'data(label)',
      'overlay-color': '#f7991f',
      'overlay-opacity': 0.25,
      shape: 'roundrectangle',
      color: '#fafafa',
    },
  },
  {
    selector: '.availabilityZone:active',
    css: {
      label: 'data(label)',
      'overlay-color': '#f7991f',
      'overlay-opacity': 0.45,
      shape: 'roundrectangle',
      color: '#fafafa',
    },
  },
  {
    selector: '.availabilityZone',
    css: {
      label: 'data(label)',
      'font-family': 'Amazon Ember, Helvetica, Arial, sans-serif',
      'font-size': '7rem',
      'text-valign': 'top',
      'text-halign': 'center',
      'background-color': function (ele: any) {
        return ele.data('color') ?? '#000'
      },
      'background-opacity': function (ele: any) {
        return ele.data('opacity') ?? 1
      },
      'border-style': 'dashed',
      'border-width': 1,
      'border-color': '#f7991f',
      'text-margin-y': '-4.5%' as any,
      'text-border-width': '2px' as any,
      'text-background-opacity': 1,
      'text-border-color': '#f7991f',
      'text-background-color': '#f7991f',
      'text-background-shape': 'rectangle',
      'text-background-padding': '5px',
      shape: 'rectangle',
      color: '#000',
    },
  },
  {
    selector: '.account:active',
    css: {
      label: 'data(label)',
      'overlay-color': function (ele: any) {
        return ele.data('accountColour') ?? 'rgba(234, 220, 23, 0.5)'
      },
      'overlay-opacity': 0.45,
      shape: 'roundrectangle',
      color: '#fafafa',
    },
  },
  {
    selector: '.account:selected',
    css: {
      label: 'data(label)',
      'overlay-color': function (ele: any) {
        return ele.data('accountColour') ?? 'rgba(234, 220, 23, 0.5)'
      },
      'overlay-opacity': 0.25,
      shape: 'roundrectangle',
      color: '#fafafa',
    },
  },
  {
    selector: '.account',
    css: {
      label: 'data(label)',
      'font-family': 'Amazon Ember, Helvetica, Arial, sans-serif',
      'font-size': '8rem',
      'text-valign': 'top',
      'text-halign': 'center',
      'background-color': function (ele: any) {
        return ele.data('color') ?? '#fff'
      },
      'background-opacity': function (ele: any) {
        return ele.data('opacity') ?? 1
      },
      'text-margin-y': '-5%' as any,
      'border-style': 'dashed',
      'border-width': 1,
      'border-color': function (ele: any) {
        return ele.data('accountColour') ?? 'rgba(234, 220, 23, 0.5)'
      },
      'text-border-width': '2px' as any,
      'text-background-opacity': 1,
      'text-border-color': function (ele: any) {
        return ele.data('accountColour') ?? 'rgba(234, 220, 23, 0.5)'
      },
      'text-background-color': function (ele: any) {
        return ele.data('accountColour') ?? 'rgba(234, 220, 23, 0.5)'
      },
      'text-background-shape': 'rectangle',
      'text-background-padding': '5px',
      shape: 'rectangle',
      color: '#fafafa',
    },
  },
  {
    selector: 'node.highlight',
    css: {
      label: 'data(label)',
      'font-family': 'Amazon Ember, Helvetica, Arial, sans-serif',
      'font-weight': 900,
      'text-valign': 'bottom',
      'text-halign': 'center',
      'font-size': '5rem',
      'text-margin-y': '3px' as any,
      'background-color': '#fff',
      'overlay-color': '#545B64',
      'overlay-opacity': 0.25,
      'background-opacity': 1,
      shape: 'ellipse',
      color: '#545B64',
      'border-style': function (ele: any) {
        return ele.data('borderStyle') ?? 'solid'
      },
      'border-width': 1,
      'border-color': function (ele: any) {
        return ele.data('borderColour') ?? '#545B64'
      },
      'background-width-relative-to': 'inner',
      'background-height-relative-to': 'inner',
      'background-width': '50%',
      'background-height': '50%',
      padding: '5px',
    },
  },
  {
    selector: '.resource:selected',
    css: {
      label: 'data(label)',
      'font-family': 'Amazon Ember, Helvetica, Arial, sans-serif',
      'text-valign': 'bottom',
      'text-halign': 'center',
      'font-size': '5rem',
      'text-margin-y': '3px' as any,
      'background-color': '#545B64',
      'background-opacity': 0.2,
      shape: 'ellipse',
      color: '#545B64',
      'border-style': function (ele: any) {
        return ele.data('borderStyle') ?? 'solid'
      },
      'border-width': '2',
      'border-color': function (ele: any) {
        return ele.data('borderColour') ?? '#545B64'
      },
      'background-width-relative-to': 'inner',
      'background-height-relative-to': 'inner',
      'background-width': '50%',
      'background-height': '50%',
      padding: '5px',
    },
  },
  {
    selector: '.ecsCluster:selected',
    css: {
      label: 'data(label)',
      'overlay-color': '#f7991f',
      'overlay-opacity': 0.25,
      shape: 'roundrectangle',
      color: '#fafafa',
    },
  },
  {
    selector: '.ecsCluster:active',
    css: {
      label: 'data(label)',
      'overlay-color': '#f7991f',
      'overlay-opacity': 0.45,
      shape: 'roundrectangle',
      color: '#fafafa',
    },
  },
  {
    selector: '.ecsCluster',
    css: {
      label: 'data(label)',
      'font-family': 'Amazon Ember, Helvetica, Arial, sans-serif',
      'font-size': '7rem',
      'text-valign': 'top',
      'text-halign': 'center',
      'background-color': function (ele: any) {
        return ele.data('color') ?? '#000'
      },
      'background-opacity': function (ele: any) {
        return ele.data('opacity') ?? 1
      },
      'border-style': 'dashed',
      'border-width': 1,
      'border-color': '#f7991f',
      'text-margin-y': '-4.5%' as any,
      'text-border-width': '2px' as any,
      'text-background-opacity': 1,
      'text-border-color': '#f7991f',
      'text-background-color': '#f7991f',
      'text-background-shape': 'rectangle',
      'text-background-padding': '5px',
      shape: 'rectangle',
      color: '#000',
    },
  },
  {
    selector: 'edge',
    css: {
      'curve-style': 'bezier',
      'target-arrow-shape': 'none',
      width: '1px',
      'line-style': 'solid',
      'line-color': '#545B64',
    },
  },
  {
    selector: 'edge.hidden',
    css: {
      width: '1px',
      'line-opacity': 0,
    },
  },
  {
    selector: 'edge.highlight',
    css: {
      'curve-style': 'bezier',
      'target-arrow-shape': 'none',
      width: '2px',
      'line-style': 'dashed',
      'line-color': '#545B64',
    },
  },
  {
    selector: 'edge.cy-expand-collapse-meta-edge',
    css: {
      'curve-style': 'unbundled-bezier',
      'control-point-distances': '0 0 0',
      'line-style': 'dashed',
      'line-color': '#ec7211',
      width: '2px',
    },
  },
  {
    selector: 'node.cy-expand-collapse-collapsed-node',
    css: {
      label: 'data(label)',
      'font-size': '6rem',
      'font-family': 'Amazon Ember, Helvetica, Arial, sans-serif',
      'text-valign': 'top',
      'text-halign': 'center',
      'background-color': '#fff',
      'background-opacity': 1,
      'border-style': 'solid',
      'border-width': 1,
      'border-color': '#545B64',
      'text-border-width': '1px' as any,
      'text-background-opacity': 1,
      'text-border-color': '#545B64',
      'text-background-color': '#545B64',
      'text-background-shape': 'rectangle',
      'background-image': function (ele: any) {
        return ele.data('image') ?? 'none'
      },
      'background-fit': 'none',
      width: '43px',
      height: '43px',
      shape: 'ellipse',
      color: '#fff',
      'background-width': '50%',
      'background-height': '50%',
    },
  },
  {
    selector: '.cloudmap:selected',
    css: {
      label: 'data(label)',
      'overlay-color': '#f7991f',
      'overlay-opacity': 0.25,
      shape: 'roundrectangle',
      color: '#fafafa',
    },
  },
  {
    selector: '.cloudmap:active',
    css: {
      label: 'data(label)',
      'overlay-color': '#f7991f',
      'overlay-opacity': 0.45,
      shape: 'roundrectangle',
      color: '#fafafa',
    },
  },
  {
    selector: '.cloudmap',
    css: {
      label: 'data(label)',
      'font-family': 'Amazon Ember, Helvetica, Arial, sans-serif',
      'font-size': '7rem',
      'text-valign': 'top',
      'text-halign': 'center',
      'background-color': function (ele: any) {
        return ele.data('color') ?? '#000'
      },
      'background-opacity': function (ele: any) {
        return ele.data('opacity') ?? 1
      },
      'border-style': 'dashed',
      'border-width': 1,
      'border-color': '#f7991f',
      'text-margin-y': '-4.5%' as any,
      'text-border-width': '2px' as any,
      'text-background-opacity': 1,
      'text-border-color': '#f7991f',
      'text-background-color': '#f7991f',
      'text-background-shape': 'rectangle',
      'text-background-padding': '5px',
      shape: 'rectangle',
      color: '#000',
    },
  },
]
