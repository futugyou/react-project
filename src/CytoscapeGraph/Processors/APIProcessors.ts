...
const isCloudMap = R.includes(R.__, [
  'AWS_ServiceDiscovery_Namespace',
...
export const processElements = ({ nodes, edges }: ConfigResourceData) => {
  ...
  const subnetBoundingBoxes = nodes.reduce((acc, { properties }) => {
    ...
    if (resourceType === 'AWS::EC2::Subnet') {
      ...
      if (
        availabilityZone != '' &&
        !['Not Applicable', 'Regional', 'Multiple Availability Zones'].includes(availabilityZone) &&
        !acc.has(azId)
      ) {
        acc.set(
          azId,
  ...
  const ecsBoundingBoxes = nodes.reduce((acc, { properties }) => {
    ...
    if (resourceType === 'AWS::ECS::Task') {
      ...
      if (!acc.has(ecsvpc)) {
        const vpc = Array.from(vpcBoundingBoxes.values()).find(
          (p) => p.data?.properties?.resourceId == vpcId,
        )
        if (vpc) {
          acc.set(
            ecsvpc,
      ...
      if (!acc.has(ecssunbet)) {
        const subnet = Array.from(subnetBoundingBoxes.values()).find(
          (p) => p.data?.properties?.resourceId == subnetId,
        )
        if (subnet) {
          acc.set(
            ecssunbet,
  ...
  return [
    ...
    ...edges
      .filter(
        (x) => !(isSubnetOrVpc(x.target.resourceType) || isSubnetOrVpc(x.source.resourceType)),
      )
...
const removeEmptyBoundingBoxes = (
