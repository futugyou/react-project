const getGreenState = (state: string) => {
  return new RegExp(
    ['available', 'running', 'in-use', 'active', 'successful', 'issued'].join('|'),
  ).exec(state)
}

const getRedState = (state: string) => {
  return new RegExp(
    ['stopped', 'deleted', 'shutting-down', 'terminated', 'stopping', 'failed', 'disabled'].join(
      '|',
    ),
  ).exec(state)
}

const getAmberState = (state: string) => {
  return new RegExp(['creating', 'pending', 'provisioning'].join('|')).exec(state)
}

export const getStateInformation = (state: string) => {
  try {
