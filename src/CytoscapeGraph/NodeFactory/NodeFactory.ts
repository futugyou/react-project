...
import {
  ...
  getAccountColour,
  getRegionColour,
} from '@/CytoscapeGraph/NodeFactory/ImageSelector'
...
export const buildBoundingBox = (
  { id, type, label, properties }: buildBoundingBoxProps,
  parent?: string,
) => {
  try {
    const boundingBox: cytoscape.ElementDefinition = {
      ...
      data: {
        ...
        clickedId: id,
        cost: Number(0),
        accountColour: getAccountColour(type === 'account' ? label : 'global'),
        regionColour: getRegionColour(type === 'region' ? label : 'Multi-Region'),
        aZColour: '#00A1C9',
  ...
  try {
    ...
    const builtNode: cytoscape.ElementDefinition = {
      ...
      data: {
        ...
        shape: 'image',
        type: 'resource',
        accountColour: getAccountColour(properties.accountId ? properties.accountId : 'global'),
        regionColour: getRegionColour(properties.awsRegion ? properties.awsRegion : 'Multi-Region'),
