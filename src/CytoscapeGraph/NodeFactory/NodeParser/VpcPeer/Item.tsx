...
export const Item = (configuration: any) => {
  ...
  const li1 = parsedConfig.accepterVpcInfo.cidrBlockSet.map((p: any) => (
    <li key={p.cidrBlock}>{p.cidrBlock}</li>
  ))
  const li2 = parsedConfig.requesterVpcInfo.cidrBlockSet.map((p: any) => (
    <li key={p.cidrBlock}>{p.cidrBlock}</li>
  ))
  return (
    <ColumnLayout columns={2} variant="text-grid">
      <SpaceBetween size="l">
        ...
        <ValueWithLabel label="accepter-vpcId">{parsedConfig.accepterVpcInfo.vpcId}</ValueWithLabel>
      </SpaceBetween>
      <SpaceBetween size="l">
