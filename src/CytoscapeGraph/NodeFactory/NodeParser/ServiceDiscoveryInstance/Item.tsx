...
export const Item = (configuration: any) => {
  ...
  return (
    <ColumnLayout columns={2} variant="text-grid">
      <SpaceBetween size="l">
        <ValueWithLabel label="InstanceId">{parsedConfig.InstanceId}</ValueWithLabel>
        <ValueWithLabel label="ServiceId">{parsedConfig.ServiceId}</ValueWithLabel>
      </SpaceBetween>
      <SpaceBetween size="l">
        ...
        <ValueWithLabel label="REGION">{parsedConfig.InstanceAttributes.REGION}</ValueWithLabel>
      </SpaceBetween>
    </ColumnLayout>
  )
}
