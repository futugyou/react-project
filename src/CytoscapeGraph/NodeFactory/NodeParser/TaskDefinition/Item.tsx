...
export const Item = (configuration: any) => {
  ...
  const images = parsedConfig.ContainerDefinitions?.map((p: any) => (
    <li key={p.Image}>{p.Image}</li>
  ))
  ...
  return (
    <ColumnLayout columns={1} variant="text-grid">
      <SpaceBetween size="l">
        <ValueWithLabel label="Family">{parsedConfig.Family}</ValueWithLabel>
        <ValueWithLabel label="ExecutionRoleArn">{parsedConfig.ExecutionRoleArn}</ValueWithLabel>
        <ValueWithLabel label="TaskDefinitionArn">{parsedConfig.TaskDefinitionArn}</ValueWithLabel>
        <ValueWithLabel label="TaskRoleArn">{parsedConfig.TaskRoleArn}</ValueWithLabel>
