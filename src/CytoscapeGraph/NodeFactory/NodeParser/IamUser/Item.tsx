...
export const Item = (configuration: any) => {
  ...
  const ups = parsedConfig.userPolicyList.map((p: any) => (
    <li key={p.policyName}>{p.policyName}</li>
  ))
  ...
  return (
    <ColumnLayout columns={2} variant="text-grid">
      <SpaceBetween size="l">
        ...
        <ValueWithLabel label="userId">{parsedConfig.userId}</ValueWithLabel>
        <ValueWithLabel label="userName">{parsedConfig.userName}</ValueWithLabel>
