...
const NewsForCompany = () => {
  ...
  useEffect(() => {
    if (nodeData && !isError) {
      const data = filter(
        nodeData,
        (f) => f.Title.includes(filteringText) || f.Summary.includes(filteringText),
      )
      ...
      var tmp = orderBy(data, 'TimePublished', 'desc')

      tmp = take(drop(tmp, (page - 1) * pageSize), pageSize)
  ...
  return (
    <div data-style="board-style">
      <Cards
        cardDefinition={{
          header: (item) => (
            <Link href={item.URL} external={true}>
              {item.Title}
            </Link>
          ),
          sections: [
            ...
            },
            {
              ...
              content: (item) => (
                <Link href={'https://' + item.SourceDomain} external={true}>
                  {item.Source}
                </Link>
              ),
            },
            ...
            },
            {
              ...
              content: (item) => (
                <SpaceBetween direction="horizontal" size="xs">
                  {item.TickerSentiment?.map((s: string) => (
                    <Badge key={s}>{s}</Badge>
                  ))}
                </SpaceBetween>
              ),
        ...
        pagination={<Paging Page={page} PageCount={pagesCount} OnPageChange={HandlePageChange} />}
        filter={
          <TextFilter
