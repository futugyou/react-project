...
const BaseEconomicChart = (props: IBaseEconomicChartProp) => {
  ...
  const [selectedTimeIntervalsOption, setSelectedTimeIntervalsOption] = useState(
    props.TimeIntervals[0],
  )
  ...
  let yTitle: string =
    props.ChartName +
    ' ' +
    selectedTimeIntervalsOption.label +
    ' Data (' +
    selectedUnitTypesOption.label +
    ')'
  ...
  const HandleUnitTypesChange = useCallback(
    (event: NonCancelableCustomEvent<SelectProps.ChangeDetail>) => {
  ...
  useEffect(() => {
        ...
        const d = map(
          orderBy(
            filter(
              ds,
              (a) =>
                a.Interval == selectedTimeIntervalsOption.value &&
    ...
    props.Data,
    props.IsLoading,
    selectedTimeIntervalsOption,
    selectedUnitTypesOption,
    startDate,
    endDate,
  ])
