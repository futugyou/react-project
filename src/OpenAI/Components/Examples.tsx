...
const ModalExampleDetail = (props: any) => {
  return (
    <MiniModal show={props.showModal} setShow={props.setShowModal} size="middle">
      <ExampleDetail
        data={props.exampleData}
        onEidtClick={() => props.onModeChange(true)}
      ></ExampleDetail>
    </MiniModal>
  )
}
const ModalExampleEdit = (props: any) => {
  return (
    <MiniModal show={props.showModal} setShow={props.setShowModal} size="middle">
      <ExampleEdit
        data={props.exampleData}
        onCancelClick={() => props.onModeChange(false)}
...
const Examples = (props: any) => {
  ...
  const [exampleData, setExampleData] = useState<ExampleModel>(
    exampleList.length > 1 ? exampleList[0] : DefaultExampleModel,
  )
    ...
    const getAllExamples = async () => {
      ...
      const list = lista
        .filter(
          (p: ExampleModel) =>
            p.tags == undefined ||
            p.tags.findIndex((element) => {
              return (
                element.toLowerCase() === searchFilter.category.toLowerCase() ||
  ...
  return (
    <div className="example-page">
      <ModalExampleDetail
        key={exampleData.key + 'Detail'}
        showModal={showModal && !editMode}
        setShowModal={setShowModal}
        onModeChange={onModeChange}
        exampleData={exampleData}
      ></ModalExampleDetail>
      <ModalExampleEdit
        key={exampleData.key + 'Edit'}
        showModal={showModal && editMode}
        setShowModal={setShowModal}
        onModeChange={onModeChange}
        exampleData={exampleData}
      ...
      <div className="example-container">
        ...
        <div className="example-item-container">{exampleItems}</div>
      </div>
    </div>
  )
}
