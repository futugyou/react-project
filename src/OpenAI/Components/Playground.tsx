...
const Playground = () => {
  ...
  const disabled =
    currentData == null || currentData.createdAt == playgroundModel.createdAt ? false : true
  const opertionContainerClassName = disabled
    ? 'opertion-container playground-disabled'
    : 'opertion-container'
  ...
  const handleCompletionStream = async () => {
    let data: OpenAIModel = convert.mapPlaygroundModelToOpenAIModel(playgroundModel, mode)
    await completionService.createCompletionStream(data, handleStreamProcess, () =>
      handleStreamEnd(data.prompt),
    )
  }
  ...
  const handleInsertStream = async () => {
    ...
    await completionService.createCompletionStream(
      data,
      (d: string) => handleInsertStreamProcess(d, data),
      () => handleInsertStreamEnd(data),
    )
  }
