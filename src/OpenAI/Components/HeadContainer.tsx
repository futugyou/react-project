...
const HeadContainer = (props: any) => {
  ...
  const selects: DropdownItem[] = [{ key: '', value: 'Load a preset...' }].concat(
    examples.concat(customExamples).map((data) => {
      return {
        key: data.key,
        value: data.title,
      }
  ...
  const HandleSelectChange = (value: string) => {
    if (props.onPresetChange) {
      ...
      if (value == '') {
        ...
        playground =
          example == null ? DefaultPlayground : convert.mapExampleModelToPlaygroundModel(example)
      }
  ...
  return (
    <div className="playground-header">
      {showSuccess && (
        <div className="alert alert-success setting-change-tip" role="alert">
          Settings updated!
        </div>
      )}
      ...
      <div className="playground-header-right">
        ...
        <div
          className="playground-header-save"
          data-bs-toggle="modal"
          data-bs-target="#exampleSave"
        >
          <span>Save</span>
        </div>
      </div>
