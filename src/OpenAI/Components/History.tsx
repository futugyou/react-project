...
const History = (props: any) => {
  ...
  const currentItems = (
    <>
      ...
      <li
        className={
          historySelected === currentEdit.createdAt
            ? 'history-item-wrap history-selected history-current'
            : 'history-item-wrap history-current'
        }
        onClick={() => handleRecordClick(currentEdit)}
      >
  ...
  const historyItems = historyList.map((data, index) => {
    ...
    return (
      <Fragment key={index}>
        ...
        <li
          className={
            historySelected === data.createdAt
              ? 'history-item-wrap history-selected'
              : 'history-item-wrap'
          }
