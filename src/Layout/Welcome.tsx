...
const Welcome = () => {
  ...
  const welcomeitem = items.map((i, index) => {
    ...
    return (
      <div key={i.title} className="welcome-item">
        <img style={{ width: '100%' }} src={i.image} alt={i.title} />
        <div className="welcome-desc">
          <Link onFollow={() => openExternalUrl(href)} external={true} fontSize="heading-m">
            {i.title}
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="welcome-container-bound">
      <div className="welcome-container">{welcomeitem}</div>
    </div>
  )
}
