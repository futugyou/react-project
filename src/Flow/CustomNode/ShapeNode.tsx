...
const ShapeNode = (props: NodeProps<ShapeNodeData>) => {
  ...
  useEffect(() => {}, [props.xPos, props.yPos])
  ...
  return (
    <div className={styles.ShapeNode}>
      {(props.data?.op?.allowResizer ?? true) && (
        <NodeResizer
          minWidth={30}
          minHeight={30}
          onResize={onResize}
          ...
          lineClassName={styles.ShapeNodeResizerLine}
          handleClassName={styles.ShapeNodeResizerHandle}
        />
      )}

      <HandlePlus id={props.id} selected={props.selected}></HandlePlus>
      <div className={styles.nodeDisplayContainer}>
        <div className={styles.nodeDisplayLable} style={{ color: node.style?.color }}>
          {props.data.label}
        </div>
      </div>
