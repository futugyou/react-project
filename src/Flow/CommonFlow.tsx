          ...
          if (p.id == node.id) {
            ...
            const position: XYPosition = {
              x:
                ...
                (parent.positionAbsolute?.x ?? parent.position.x),
              y:
                (node.positionAbsolute?.y ?? node.position.y) -
                (parent.positionAbsolute?.y ?? parent.position.y),
            }
            return {
              ...node,
  ...
  return (
    <div style={{ width: '100%', height: '100%' }} ref={reactFlowWrapper}>
      ...
      >
        ...
        {showControl && selectedEdge && (
          <EdgeStyle selectedEdge={selectedEdge} key={selectedEdge?.id ?? getRandomId()} />
        )}
        {showControl && selectedNode && (
          <NodeStyle selectedNode={selectedNode} key={selectedNode?.id ?? getRandomId()} />
        )}
