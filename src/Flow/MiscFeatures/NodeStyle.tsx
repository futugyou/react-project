...
const NodeStyle = (props: NodeStyleProps) => {
  ...
  const [resizer, setResizer] = useState<boolean>(
    selectedNode?.data?.op?.allowResizer == undefined ? true : selectedNode?.data?.op?.allowResizer,
  )
  const [keepAspectRatio, setKeepAspectRatio] = useState<boolean>(
    selectedNode?.data?.op?.keepAspectRatio == undefined
      ? false
      : selectedNode?.data?.op?.keepAspectRatio,
  )
  ...
  const ChangeRatioState = () => {
    const data = {
      ...selectedNode.data,
      op: { ...selectedNode?.data?.op, keepAspectRatio: !keepAspectRatio },
    }
      ...
      <div className={styles.groupContainer}>
        <div className={styles.groupLayerContainer}>
          ...
          <div className={styles.groupLayer}>
            <div
              className={`${styles.groupLayerItem} ${borderStyle == 'none' ? styles.selected : ''}`}
              onClick={() => onChangeBorder('none')}
            >
            ...
            <div
              className={`${styles.groupLayerItem} ${borderStyle == 'solid' ? styles.selected : ''}`}
              onClick={() => onChangeBorder('solid')}
            >
              <svg width="40" height="40" xmlns="http://wwww.w3.org/2000/svg">
                <title>solid</title>
            ...
            <div
              className={`${styles.groupLayerItem} ${borderStyle == 'dashed' ? styles.selected : ''}`}
              onClick={() => onChangeBorder('dashed')}
            >
              <svg width="40" height="40" xmlns="http://wwww.w3.org/2000/svg">
                <title>dashed</title>
            ...
            <div className={`${styles.groupLayerItem} ${styles.color}`}>
              <div
                onClick={() => onColorClick('border-color')}
                style={{
                  width: '100%',
                  height: '50%',
        ...
        <div className={styles.groupLayerContainer}>
          ...
          <div className={styles.groupLayer} style={{ height: '52px' }}>
            <div className={`${styles.groupLayerItem} ${styles.color}`}>
              <div
                onClick={() => onColorClick('background-color')}
                style={{
                  width: '100%',
                  height: '50%',
          ...
          <div className={styles.groupLayer} style={{ height: '52px' }}>
            <div className={`${styles.groupLayerItem} ${styles.color}`}>
              <div
                ...
                style={{ width: '100%', height: '50%', backgroundColor: color, color: color }}
              ></div>
            </div>
            <div className={styles.groupLayerItem} style={{ border: 0 }}></div>
            <div className={styles.groupLayerItem} style={{ border: 0 }}></div>
        ...
        {checkNodeLableDisplay(nodeType) && (
          <div className={styles.groupLayerContainer}>
            ...
            <div className={styles.groupLayer} style={{ height: '52px' }}>
              ...
              <div
                className={`${styles.groupLayerItem}`}
                style={{ border: 0, width: '52px', height: '52px' }}
              ></div>
            </div>
          </div>
        )}
