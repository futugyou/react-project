  ...
  useImperativeHandle(ref, () => ({
    execHelpLinePosition(node: Node) {
      ...
      const xs = nodes
        ...
        .map((p) => [
          p.positionAbsolute?.x!,
          p.positionAbsolute?.x! + (p.width ?? 0),
          p.positionAbsolute?.x! + (p.width ?? 0) / 2,
        ])
        .flatMap((p) => p)
      ...
      const ys = nodes
        .filter((p) => p.id !== node.id)
        .map((p) => [
          p.positionAbsolute?.y!,
          p.positionAbsolute?.y! + (p.height ?? 0),
          p.positionAbsolute?.y! + (p.height ?? 0) / 2,
        ])
        .flatMap((p) => p)
