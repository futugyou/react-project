...
const MiniModal = (props: MiniModalProps) => {
  ...
  const { play, style, isPlaying } = useAnimateKeyframes({
    ...
    direction: 'alternate',
    duration: 0.2,
    keyframes: ['transform: scale(1, 1)', 'transform: scale(1.01, 1.01)', 'transform: scale(1, 1)'],
  })
    ...
    return (
      <div className="mini-modal" onClick={() => onModalWindowClick()}>
        <div className={size} style={style} onClick={(e) => e.stopPropagation()}>
          <div className="mini-modal-close" onClick={() => props.setShow(false)}>
            X
          </div>
          <div className="mini-modal-body">{props.children}</div>
